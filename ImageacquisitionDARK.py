...
def rename_file_drag_drop(suffix):
    def inner(event):
        ...
        if not selected_item:
            show_temporary_message("No product selected.", text_color='red')
            return
        item_values = tree.item(selected_item, 'values')
        if not item_values:
            show_temporary_message("No data in the selected record.", text_color='red')
            return
        ...
        if os.path.exists(new_file_path):
            show_temporary_message(f"A file named '{new_file_name}' already exists.", text_color='red')
            return False
        try:
            os.rename(file_path, new_file_path)
            show_temporary_message(f"File renamed to {new_file_name}", text_color='green')
            return True
...
def on_delete_key(event):
    ...
    if selected_items:
        for item in selected_items:
            tree.delete(item)
        show_temporary_message(f"{len(selected_items)} items deleted.", text_color='white')
...
def rename_selected_file(suffix):
    ...
    try:
        os.rename(file_path, new_file_path)
        show_temporary_message(f"File renamed to {new_file_name}")
    except Exception as e:
...
def fetch_records(cursor, single_checked, multipack_checked):
    try:
        query = "SELECT TOP 100 ID, Name, Desc20, Desc10 FROM MC_Products WHERE Status <> 'Live'"
...
def main():
    ...
    single_check = customtkinter.CTkCheckBox(fetch_frame, text="Single", variable=single_check_var, text_color="white")
    single_check.grid(row=0, column=0, padx=10, pady=10)

    multipack_check = customtkinter.CTkCheckBox(fetch_frame, text="Multipack", variable=multipack_check_var,
                                                text_color="white")
    multipack_check.grid(row=0, column=1, padx=10, pady=10)

    fetch_button = customtkinter.CTkButton(fetch_frame, text="Filter newest products", command=lambda:
    fetch_records(cursor, single_check_var.get(), multipack_check_var.get()))
    fetch_button.grid(row=0, column=2, padx=10, pady=10)

    help_button = customtkinter.CTkButton(fetch_frame, text="Help", width=20, command=open_help_window)
    help_button.grid(row=0, column=4, padx=(360, 0))

    single_filter_var = tk.StringVar()

    batch_filter_frame = customtkinter.CTkFrame(root, height=47, fg_color="#2c2e42")
    batch_filter_frame.grid(row=2, column=0, padx=10, pady=10, sticky='ew')
    ...
    batch_filter_by_id_button.grid(row=0, column=1, padx=10, pady=5)

    batch_filter_by_mc_button = customtkinter.CTkButton(batch_filter_frame, text="Batch Filter by MC RPL UPC",
               command=lambda: batch_filter_window("MC RPL UPC"))
    batch_filter_by_mc_button.grid(row=0, column=2, padx=10, pady=5)

    clear_button = customtkinter.CTkButton(batch_filter_frame, text="Clear all products", fg_color='IndianRed1',
                                           command=clear_records, width=17)
    ...
    tree.heading("Image status", text="Image status", command=lambda: None)
    tree.column("Image status", width=105)
    tree.heading("ID", text="ID")
    tree.heading("Name", text="Name")
    tree.heading("MC_RPL_UPC", text="MC_RPL_UPC")
    tree.heading("Package Type", text="Package Type")
    ...
    export_button.grid(row=1, column=0, pady=(20, 10))
    load_button = customtkinter.CTkButton(bottom_frame_secondary, text="Load xlsx", command=load_from_excel)
    load_button.grid(row=1, column=1, padx=10, pady=(20, 10))  # Adjust grid position as needed
    pack_files_button = customtkinter.CTkButton(bottom_frame_secondary, text="Pack images in folders", command=pack_files_in_folders)
    pack_files_button.grid(row=1, column=2, pady=(20, 10))
    # Rename frame
    rename_frame = customtkinter.CTkFrame(root, width=500, height=100, fg_color="#2c2e42")
    rename_frame.grid(row=10, column=0, pady=(10, 0))
    renaming_label = customtkinter.CTkLabel(rename_frame, text="↓ Images dropped below will be renamed per orientation ↓"
                                            , text_color="white")
    ...
    front_label = tk.Label(second_bottom_frame, text="Drop for Front", bg="lightblue", width=30, height=500)
    front_label.grid(row=0, column=0, padx=10, pady=10, sticky='nsew')

    side_label = tk.Label(second_bottom_frame, text="Drop for Side", bg="lightgreen", width=30, height=500)
    side_label.grid(row=0, column=1, padx=10, pady=10, sticky='nsew')

    top_label = tk.Label(second_bottom_frame, text="Drop for Top", bg="RosyBrown2", width=30, height=500)
